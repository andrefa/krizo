...
var gravity := GameBalance.BASE_GRAVITY
var thrust := GameBalance.BASE_THRUST
var horizontal_speed := GameBalance.BASE_HORIZONTAL_SPEED
var fuel_capacity := GameBalance.BASE_FUEL_CAPACITY
var fuel_burn_per_second := GameBalance.BASE_BURN_RATE
var fuel_recharge_per_second := GameBalance.BASE_RECHARGE_RATE
var max_fall_speed := 720.0
var horizontal_acceleration := 1150.0
...
func _physics_process(delta: float) -> void:
	if not alive:
		...
		visual.rotation = lerp(visual.rotation, 0.55, 2.0 * delta)
		move_and_slide()
		return

	var horizontal := Input.get_axis("move_left", "move_right")
	if touch_active:
		var error := touch_x - global_position.x
		horizontal = clamp(error / 130.0, -1.0, 1.0)
	velocity.x = move_toward(velocity.x, horizontal * horizontal_speed, horizontal_acceleration * delta)

	var boosting := (Input.is_action_pressed("jetpack") or touch_active) and fuel > 0.0
	velocity.y = min(velocity.y + gravity * delta, max_fall_speed)
	if boosting:
	...
	move_and_slide()
	global_position.x = clamp(global_position.x, GameBalance.WORLD_LEFT, GameBalance.WORLD_RIGHT)

	var meters := current_altitude()
...
func _update_visuals(boosting: bool, delta: float) -> void:
	...
	var lean := clamp(velocity.x / max(horizontal_speed, 1.0), -1.0, 1.0) * 0.22
	visual.rotation = lerp(visual.rotation, lean, 8.0 * delta)
	var target_scale := Vector2(0.97, 1.05) if boosting else Vector2.ONE
	visual.scale = visual.scale.lerp(target_scale, 10.0 * delta)

func current_altitude() -> int:
	return max(0, int((start_y - global_position.y) / GameBalance.PIXELS_PER_METER))
